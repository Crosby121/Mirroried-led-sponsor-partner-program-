...
const basePages = [
  ['dashboard','Dashboard'],['campaigns','Campaigns'],['placements','Ad Spaces'],['deliverables','Deliverables'],['approvals','Approvals'],['assets','Creative Assets'],['proofs','Proof'],['reports','Reports'],['renewals','Renewals']
];
...
function placementCategory(name=''){
  const v=String(name).toLowerCase();
  if(v.includes('youtube')||v.includes('social')||v.includes('website')||v.includes('digital')) return 'Digital';
  if(v.includes('interior')||v.includes('showroom')) return 'Interior Showroom';
  if(v.includes('event')||v.includes('festival')||v.includes('stadium')) return 'Live Event';
  return 'Exterior / Mobile';
}

function placementFormat(item){
  const value=`${item.inventoryAssignment||''} ${item.type||''}`.toLowerCase();
  if(value.includes('mirror')) return 'Infinity / Sponsor Mirror';
  if(value.includes('video')||value.includes('led')) return 'LED / Video Display';
  if(value.includes('social')||value.includes('youtube')) return 'Digital Content';
  return item.type||'Campaign Placement';
}

function renderPlacements(items){
  const assigned=items.filter(i=>String(i.inventoryAssignment||'').trim());
  if(!assigned.length){
    $('#content').innerHTML=`<section class="panel"><h2>Advertising Spaces</h2><div class="empty">No advertising spaces have been assigned to campaigns in this account yet.</div></section>`;
    return;
  }
  $('#content').innerHTML=`<section class="panel"><h2>Advertising Spaces</h2><p class="muted">Approved placements assigned to campaign deliverables. Sponsor-facing results remain account scoped by the production backend.</p><div class="placement-grid">${assigned.map(item=>`<article class="placement-card"><div>${badge(item.status)}</div><h3>${esc(item.inventoryAssignment)}</h3><p>${esc(item.name)}${item.notes?`<br>${esc(item.notes)}`:''}</p><div class="placement-meta"><span>${esc(placementCategory(item.inventoryAssignment))}</span><span>${esc(placementFormat(item))}</span></div></article>`).join('')}</div></section>`;
}

async function loadPage(){
  const titles={dashboard:'Dashboard',opportunities:'Opportunities',campaigns:'Campaigns',placements:'Advertising Spaces',deliverables:'Deliverables',approvals:'Approvals',assets:'Creative Assets',proofs:'Proof of Delivery',reports:'Reports',renewals:'Renewals'};
  $('#pageTitle').textContent=titles[state.page]||'Portal';
  $('#content').innerHTML='<div class="loading">Loading…</div>';
  try{
    if(state.page==='dashboard'){
      const d=await api('/api/dashboard');
      const placements=d.deliverables.filter(x=>String(x.inventoryAssignment||'').trim());
      $('#content').innerHTML=cards(d.summary)+`<div class="grid-2"><section class="panel"><h2>Campaigns</h2>${table(d.campaigns,[['Campaign','name'],['Status','status',badge],['Start','startDate'],['End','endDate']])}</section><section class="panel"><h2>Needs attention</h2>${table(d.approvals,[['Approval','title'],['Status','status',badge],['Requested','requestedAt']])}</section></div><section class="panel"><h2>Current Ad Placements</h2>${table(placements,[['Placement','inventoryAssignment'],['Deliverable','name'],['Status','status',badge],['Start','plannedStart'],['End','plannedEnd']])}</section><section class="panel"><h2>Deliverables</h2>${table(d.deliverables,[['Deliverable','name'],['Status','status',badge],['Start','plannedStart'],['End','plannedEnd'],['Placement','inventoryAssignment']])}</section>`;
      return;
    }
    const apiPage=state.page==='placements'?'deliverables':state.page;
    const data=await api(`/api/${apiPage}`);
    ...
    if(state.page==='placements') renderPlacements(items);
...
function renderAssets(items){
  ...
  $('#content').innerHTML=`<div class="grid-2"><section class="panel"><h2>Creative library</h2>${rows}</section><section class="panel"><h2>Upload campaign creative</h2><form id="assetForm" class="stack"><label>Campaign ID<input id="assetCampaign" required placeholder="cmp-..."></label>${['sales','operations','admin'].includes(state.user.role)?'<label>Sponsor ID<input id="assetSponsor" required placeholder="spn-..."></label>':''}<label>File<input id="assetFile" type="file" required></label><label>Asset name<input id="assetName" placeholder="Logo / campaign creative"></label><button type="submit">Upload asset</button><p class="muted">Max file size: 2 MB. Files remain behind authenticated sponsor-scoped routes.</p></form></section></div>`;
...
async function boot(){
  ...
  try{const me=await api('/api/me');state.user=me.user;showApp();loadPage();}catch{showMarketing();}
}

$('#publicLoginBtn').onclick=showLogin;
$('#heroLoginBtn').onclick=showLogin;
$('#ctaLoginBtn').onclick=showLogin;
$('#backToMarketing').onclick=showMarketing;
...
$('#logoutBtn').onclick=async()=>{await api('/api/logout',{method:'POST'}).catch(()=>{});state.user=null;state.page='dashboard';showMarketing();};
